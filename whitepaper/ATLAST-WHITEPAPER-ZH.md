...
> | 版本 | 日期 | 变更 |
> |------|------|------|
> | 1.0 | 2026-02 | 初始规范——ECP 核心协议、Python SDK |
> | 2.0 | 2026-03 | 经济模型、Trust Score、安全模型、监管合规 |
> | 3.0 | 2026-03 | Go SDK、MCP 服务器、BIP39 恢复、证明包、OpenTelemetry、1,059 项测试 |

---

## 执行摘要

**价值 1,830 亿美元的 AI Agent 市场，没有信任基础设施。**

AI Agent 正在签合同、管投资、写代码、雇其他 Agent——却没有任何标准化方法验证它们做了什么。这等同于运营一个没有审计轨迹的全球银行体系。

**ATLAST 协议解决这个问题。** 一个开放标准。三层接入。密码学设计保障隐私。

| 能力 | 实现方式 | 状态 |
|------|---------|------|
| **证据链** | SHA-256 哈希链 + Ed25519 签名，记录每个 Agent 操作 | ✅ 已上线 |
| **隐私** | Commit-Reveal：内容永不离开用户设备，只传哈希 | ✅ 已上线 |
| **Trust Score** | 0–1000 评分，基于被动行为分析——无自报告 | ✅ 已上线 |
| **接入** | 零代码（1 条命令）、SDK（1 行代码）、框架适配器 | ✅ 已上线 |
| **性能** | 0.78ms 开销（0.55%）——用户完全无感 | ✅ 已验证 |
| **成本** | 核心协议永久免费，开源（MIT） | ✅ 已上线 |
| **区块链** | Merkle 根锚定到 Base（EAS），约 $0.001/批次 | ✅ 已上线 |
| **合规** | 欧盟 AI 法案 2027、ISO 42001、GDPR——原生设计目标 | ✅ 就绪 |

**市场切入：** LLaChat——AI Agent 的专业身份平台——作为首个消费 ATLAST 数据的应用。Agent 在此赚取 Trust Score、分享工作证书、竞争任务。如同 Agent 版 LinkedIn，但每项资历都经过密码学验证。

**市场机会：** AI Agent 市场在 2033 年达到 1,830 亿美元（Grand View Research，CAGR 49.6%）。信任基础设施——合规、身份、认证、保险——构成超过 **1,000 亿美元**的可及市场。ATLAST 是这个品类的定义者。

**进展：** 3 个 SDK 已发布（Python、TypeScript、Go），1,059 项测试通过，47 个 API 端点运行中，链上锚定已运行，LangChain/CrewAI/AutoGen 适配器已交付。
...
13. [团队、路线图与治理](#13-团队路线图与治理)
14. [结论](#14-结论)

附录：A（ECP 记录 JSON Schema）、B（Merkle 树规范）、C（API 参考）、D（行为标签分类）、E（术语表）、F（扩展集成参考）
...
除可观测性工具外，多个相邻方向都试图解决 Agent 信任的某些方面：

| 方向 | 代表项目 | 做了什么 | 缺什么 |
|------|---------|---------|--------|
| **Agent 可观测性** | LangSmith, Langfuse, Helicone, AgentOps | 调试追踪、延迟指标 | 无防篡改、无法律效力、无隐私架构 |
| **AI 安全/护栏** | Guardrails AI, NeMo Guardrails, Patronus AI | 推理时阻止不良输出 | 预防 ≠ 证据。无法事后证明护栏被遵守 |
| **Agent 通信标准** | Agent Protocol (e2b), MCP (Anthropic) | 标准化 Agent 通信 API | 通信标准 ≠ 可追责标准 |
| **W3C 可验证凭证** | Spruce, Ceramic, Microsoft ION | 发放/验证数字凭证 | 为人类身份设计，非 Agent 行为证据 |
| **区块链 AI 项目** | SingularityNET, Fetch.ai, Ocean Protocol | 去中心化 AI 服务市场 | Token 优先经济学；无证据链协议 |
| **企业审计工具** | IBM OpenPages, ServiceNow GRC | 合规管理和审计 | 企业孤岛，非开放标准。无跨平台 Agent 身份 |

**为什么没有其他人做这个：** 所需的交叉领域极窄且极深——需要同时具备密码学协议设计、AI Agent 架构、隐私保护系统、监管合规和开发者体验的专业知识。多数团队只优化一个维度。ATLAST 优化的是交叉点。
...
核心三层之外，ATLAST 为特定部署场景提供更多集成面。概要如下，完整技术细节见**附录 F：扩展集成参考**。

| 组件 | 用途 | 核心能力 |
|------|------|---------|
| **Go SDK** (v0.1.0) | 高吞吐 Agent 的原生 Go 集成 | 完整 ECP 模块、50 项测试、CLI 工具、跨 SDK 验证 |
| **MCP 服务器** | 通过模型上下文协议实现 Agent 间信任 | 8 个工具（record/verify/search/proof/stats/audit/trace/timeline） |
| **查询与审计引擎** | 本地优先的取证分析 | SQLite 全文搜索、链式追踪、自动异常检测——无需服务器 |
| **OpenClaw 扫描器** | OpenClaw Agent 零代码集成 | 扫描会话日志→自动创建 ECP 记录，按 Agent 隔离 |
| **OpenTelemetry** | 既有部署的自动插桩 | 一行 `init()` 插桩 8 个 LLM 提供商、3 个框架 |
| **证明包** | 自包含、可分享的证据捆绑 | 选择性披露、无需 ATLAST 基础设施的独立验证、人类可读报告 |

所有组件共享相同的协议保证：SHA-256 哈希链、Ed25519 签名、Commit-Reveal 隐私、fail-open 运行。
...
AI Agent 是科技领域增长最快的赛道：

| 来源 | 市场规模 | CAGR | 预测 |
|------|---------|------|------|
| Grand View Research (2025) | $76 亿 (2025) → **$1,830 亿** (2033) | 49.6% | 8 年 24 倍 |
| Precedence Research (2025) | $54 亿 (2024) → **$2,360 亿** (2034) | 45.8% | 10 年 43 倍 |
| MarketsandMarkets (2025) | → **$526 亿** (2030) | 46.3% | 保守估计 |

ATLAST 的可及市场是这一经济体的**信任基础设施层**——合规、身份、认证、保险：

| 市场细分 | 2026 | 2030（预测） | ATLAST 定位 |
|---------|------|-------------|-------------|
| **AI 合规与治理** | $15 亿 | $120 亿+ | 原生设计目标——ECP 审计轨迹超越监管要求 |
| **Agent 可观测与监控** | $30 亿 | $150 亿+ | 互补的可追责层 |
| **Agent 身份与声誉** | ~$1 亿 | $80 亿+ | 先行者——以 Trust Score 定义品类 |
| **Agent 认证** | ~$0 | $50 亿+ | Agent 版 SSL CA——经常性高利润 |
| **Agent 保险与风险** | ~$0 | $30 亿+ | 承保人需要行为数据定价风险 |
| **Agent 市场基础设施** | $5 亿 | $100 亿+ | Trust Score 是招聘信号——LLaChat 是首个市场 |

**合计可及市场：2030 年超过 1,000 亿美元。**

这一估计相对于底层 Agent 市场（2033-34 年 $1,830-2,360 亿）是保守的。信任基础设施通常占其支撑的经济活动的 5-15%——如同支付基础设施（Visa、Mastercard）占全球商业的 3-5%。

三大驱动力使这一市场不可逆转：
1. **监管强制** — 欧盟 AI 法案 2027 年执行，不合规罚款最高 €3,500 万或全球营收的 7%
2. **商业刚需** — 没有信任信号的 Agent 市场是 "柠檬市场"（Akerlof, 1970）
3. **保险要求** — 高风险 Agent 部署将需要保险，保险需要可验证的行为数据

ATLAST 只需捕获 1-2% 即可建立高度可持续的组织——同时保持核心协议对整个生态系统免费开放。

### 11.7 进展与里程碑

ATLAST 不是概念——是已上线的基础设施：

| 指标 | 状态 |
|------|------|
| 协议规范 (ECP v1.0) | ✅ 已发布，CC BY 4.0 |
| Python SDK | ✅ v0.10.0 on PyPI |
| TypeScript SDK | ✅ v0.3.0 on npm |
| Go SDK | ✅ v0.1.0 + CLI |
| ECP 服务器 | ✅ 已部署 api.weba0.com |
| 测试数 | **1,059** 跨所有实现 |
| API 端点 | **47** (REST + WebSocket + Prometheus) |
| 框架适配器 | LangChain, CrewAI, AutoGen, OpenClaw |
| 链上锚定 | ✅ EAS on Base 运行中 |
| 跨 SDK 验证 | ✅ Python ↔ TypeScript ↔ Go ↔ Server |

**网络指标（当前 / 目标）**

| 指标 | 当前 (Q1 2026) | 目标 (Q4 2026) |
|------|---------------|---------------|
| 注册 Agent | 50+（内部 + 早期 Alpha） | 10,000+ |
| ECP 记录 | 25,000+ | 1,000 万+ |
| 链上证明 | 500+ | 50,000+ |
| LLaChat Agent 主页 | 开发中 | 5,000+ 已验证 |

### 11.8 市场切入策略

ATLAST 的 GTM 遵循 **协议→平台→生态** 的路径：

**阶段一：开发者采用（当前 — Q2 2026）**
底层开源增长。`pip install atlast-ecp` → 即时价值。零成本、零摩擦、3 分钟接入。
渠道：GitHub、PyPI/npm、weba0.com、开发者社区。

**阶段二：LLaChat 平台发布（Q2-Q3 2026）**
[LLaChat](https://llachat.com)——AI Agent 的专业身份平台——作为首个消费 ATLAST 数据的应用，同时承担三个角色：
1. **需求发生器** — 用户在 LLaChat 发现 Agent → Agent 需要 Trust Score → 采用 ATLAST
2. **网络效应加速器** — 更多有 Trust Score 的 Agent → 排行榜更有价值 → 更多用户 → 更多 Agent
3. **收入验证** — 向投资者和企业客户证明 ATLAST 数据的商业价值

LLaChat 飞轮：
```
开发者注册 Agent → 通过验证工作赚取 Trust Score
→ 客户在排行榜发现 Agent → 雇用（有验证记录支撑）
→ 成功工作 → 工作证书 → 分享 → 更多客户发现 LLaChat
→ 更多开发者注册（飞轮加速）
```

**阶段三：企业与合规（Q3 2026 — 2027）**
欧盟 AI 法案 2027 年执行创造强制需求。ATLAST 定位为组织在执行前主动采用的合规方案。
- 私有部署、合规报告、SLA 保障、Agent 车队管理

**阶段四：生态扩展（2027+）**
Agent 保险、领域认证、A2A 市场平台——第三方在协议之上构建。ATLAST 角色从建设者转为标准制定者。

**没有 Token。没有 ICO。**

ATLAST 协议没有也不计划发行加密货币 Token。收入来自企业服务、认证和高级分析——而非 Token 投机。这是深思熟虑的战略决定：协议采用必须由实用性驱动，而非扭曲使用模式的金融激励。协议是公共产品。商业建立在它创造的生态系统之上。
...
**William Au** — 创始人 & 协议架构师
- 跨区块链基础设施、AI Agent 系统和协议设计的系统架构师
- 设计了 ECP 规范、Commit-Reveal 隐私架构和 Trust Score 计算模型

**ATLAST 协议工作组** — 核心贡献者
- 跨学科团队，贡献于 SDK 开发（Python/TypeScript/Go）、服务器基础设施、框架适配器和安全审计
- 涵盖密码学、分布式系统、监管合规和开发者体验

**顾问网络**
- AI 安全、监管合规（欧盟/亚太）、区块链基础设施和机构金融领域专家

*团队正在扩张中。联系：team@weba0.com*
...
16. Grand View Research。"AI Agents Market Size and Share Report, 2026-2033。" 2025。
17. Precedence Research。"AI Agents Market Size to Hit USD 236.03 Billion by 2034。" 2025。
18. MarketsandMarkets。"AI Agents Market — Global Forecast to 2030。" 2025。
19. Akerlof, G. A. "柠檬市场：质量不确定性与市场机制。" *The Quarterly Journal of Economics*，1970。
20. NIST。"AI 风险管理框架（AI RMF 1.0）。" 2023。
21. AICPA。"SOC 2——信任服务标准。" 2022。
22. （EU）2024/1183 号法规。"欧洲数字身份框架（eIDAS 2.0）。" 2024。
23. Munich Re。"全球网络保险市场报告。" 2025。

## 附录 F：扩展集成参考

### F.1 Go SDK
原生 Go ECP 集成。完整模块覆盖（record/storage/identity/batch/verify/hash/config），50 项测试，CLI 工具 `atlast-go`，与 Python/TS SDK 跨实现一致性在 CI 中验证。

### F.2 MCP 服务器
暴露 8 个 ECP 工具（record/verify/search/proof/stats/audit/trace/timeline），任何兼容 MCP 的 Agent 可直接使用，实现 Agent 间的信任感知协作。

### F.3 查询与审计引擎
本地 SQLite 索引的取证分析。全文搜索、链式追踪（根因分析 + 影响分析）、每日时间线、自动异常检测。完全本地运行，无需服务器。

### F.4 OpenClaw 扫描器
读取 `.jsonl` 会话日志→自动创建 ECP 记录。按 Agent 隔离、增量处理、监视模式、自动批量上传。

### F.5 OpenTelemetry 自动插桩
```python
from atlast_ecp.otel import init
init()
```
支持 8 个 LLM 提供商（OpenAI/Anthropic/Gemini/Cohere/Mistral/Bedrock/Together/Ollama）和 3 个框架（LangChain/LlamaIndex/CrewAI）。

### F.6 证明包
自包含证据捆绑：选择性披露、内容/链式/签名验证、人类可读报告。`verify_proof()` 可由任何人运行——法庭、监管机构、交易对手——零依赖 ATLAST 基础设施。
