...
> | 2.3 | 2026-03-23 | Trust Score 0-1000 独立标准、LLaChat 参考应用定位、A2A 市场设计、维度映射、chain_integrity Phase 1 说明 |
> | 3.0 | 2026-03-29 | Go SDK、MCP 服务器、查询与审计引擎、BIP39 身份恢复、证明包、OpenClaw 扫描器、OpenTelemetry 自动插桩、Agent 注册安全（DID 防劫持）、API 密钥管理、测试数 536→1,059、端点表 v2 |
...
v3.0 新增：§6.6 Go SDK、§6.7 MCP 服务器、§6.8 查询与审计引擎、§6.9 OpenClaw 扫描器、§6.10 OpenTelemetry、§6.11 证明包、§8.3 BIP39 恢复、§8.4 DID 防劫持、§8.5 API 密钥管理
...
除 Python 和 TypeScript 外，完整的 **Go SDK**（`atlast-go`）为基于 Go 的 Agent 系统提供原生 ECP 集成：

- **完整模块覆盖：** `record`、`storage`、`identity`、`batch`、`verify`、`hash`、`config`
- **50 项测试通过**，完整 CI 集成
- **CLI 工具：** `atlast-go` 提供对所有 ECP 操作的命令行访问
- **相同协议保证：** 与 Python 和 TypeScript SDK 相同的哈希链构建、Merkle 树计算和 Ed25519 签名——跨实现一致性在 CI 中验证

Go SDK 专为性能关键的 Agent 部署设计（高频交易 Agent、基础设施管理 Agent），充分利用 Go 的并发模型和低延迟运行时。

### 6.7 MCP 服务器

SDK 包含一个 **模型上下文协议（MCP）服务器**，将 ECP 功能暴露为 8 个标准化工具，供 Agent 间访问：

| 工具 | 描述 |
|------|------|
| `record` | 创建 ECP 记录 |
| `verify` | 验证 ECP 记录或证据链 |
| `search` | 跨记录全文搜索 |
| `proof` | 生成或验证 Merkle 证明 |
| `stats` | 获取 Agent 统计信息 |
| `audit` | 运行自动化审计报告 |
| `trace` | 追踪记录链（正向/反向） |
| `timeline` | 生成每日活动时间线 |

任何兼容 MCP 的 Agent（Claude、GPT 或支持 MCP 的自定义 Agent）都可以使用这些工具来读取、验证和审计其他 Agent 的 ECP 证据链——无需自定义集成代码即可实现信任感知的 Agent 协作。

### 6.8 查询与审计引擎

SDK 包含一个**本地优先、SQLite 索引的查询引擎**，用于取证分析和合规报告：

- **全文搜索：** 按内容、标签、模型或任何元数据字段搜索所有 ECP 记录
- **链式追踪：** 反向根因分析（什么导致了这条记录？）和正向影响分析（这条记录影响了什么？）
- **每日时间线：** 按天聚合的活动、错误率和延迟趋势——用于行为漂移检测
- **自动化审计报告：** 错误峰值、置信度下降和延迟峰值的异常检测，通过链式追踪识别根因候选

查询引擎完全在本地运行——无需服务器通信。这对于组织需要取证分析能力但不能传输敏感数据的合规场景至关重要。

### 6.9 OpenClaw 扫描器

对于运行在 **OpenClaw** 平台上的 Agent，SDK 包含会话日志扫描器：

- **自动扫描：** 读取 OpenClaw `.jsonl` 会话日志，从每个 Agent 操作创建 ECP 记录
- **按 Agent 隔离：** 每个 Agent 获得自己的 DID 和隔离的 `.ecp/` 目录
- **增量处理：** 状态追踪确保不会重复处理已扫描的条目
- **监视模式：** 在活跃会话中连续扫描以实现实时 ECP 记录
- **批量上传：** 扫描的记录自动排队进行 Merkle 批量上传

这使任何 OpenClaw Agent 无需对 Agent 本身进行任何代码更改即可实现 ECP 集成。

### 6.10 OpenTelemetry 自动插桩

一行代码设置即可对所有已安装的 LLM 库进行自动 ECP 记录插桩：

```python
from atlast_ecp.otel import init
init()  # 自动插桩所有检测到的 LLM 库
```

**支持的 LLM 提供商：** OpenAI、Anthropic、Google Gemini、Cohere、Mistral、AWS Bedrock、Together AI、Ollama

**支持的框架：** LangChain、LlamaIndex、CrewAI

**架构：** 使用 OpenLLMetry 插桩器 → OpenTelemetry Span → `ECPSpanExporter` 将 OTel Span 转换为 ECP 记录。这种方法利用现有的 OpenTelemetry 生态系统，同时产出标准的 ECP 证据链。

### 6.11 证明包

**证明包**是自包含的、可独立验证的证明捆绑包，可以分享给任何人——验证无需 ATLAST 基础设施：

- **选择性披露：** 用户选择哪些记录内容包含在内，哪些进行脱敏。包含的内容可通过哈希匹配验证；脱敏内容保护隐私，同时哈希证明其存在。
- **内容验证：** 每条包含的记录 `hash(content) == stored_hash`
- **链式验证：** 重新计算链哈希以验证连接完整性
- **签名验证：** 每条记录的 Ed25519 签名验证
- **人类可读报告：** 适合法律和合规审查的格式化输出
- **向后兼容：** 同时支持 `entries` 和 `records` 字段名（别名）

关键属性：`verify_proof()` 可以由**任何人**运行——法庭、监管机构、交易对手——对 ATLAST 服务器或基础设施零依赖。证明包包含独立验证所需的一切。
...
其中：
  ...
  T(agent) = chain_integrity² × coverage_ratio
             ...
             注：Phase 1 实现中 chain_integrity = 1.0（常量），
             待链上验证基础设施完善后启用动态值。
...
Agent 身份锚定于 Ed25519 密钥对。如果私钥丢失，Agent 的身份——及其整个 Trust Score 历史——将丢失。**BIP39 助记词恢复**解决了这个问题：

- **12 词助记短语**，使用 BIP39 标准英文词库（2,048 个词）
- **确定性密钥派生：** 熵 → HKDF-SHA256 → Ed25519 种子。相同的助记词始终产生相同的密钥对，支持在任何设备上恢复。
- **传统密钥导出/导入：** 现有 Agent 可以导出身份并在新设备上重新导入
- **人类友好显示：** 助记词以编号词汇的方框格式显示，便于安全离线存储

这对 Agent 身份可移植性和灾难恢复至关重要。组织可以将所有 Agent 身份作为助记短语备份存储在安全保险库中——确保即使原始部署基础设施丢失也能保持连续性。

### 8.4 Agent 注册安全（DID 防劫持）

重新注册现有 DID 需要 **Ed25519 所有权证明**，以防止未经授权的身份接管：

- **签名消息格式：** `"register:{did}:{timestamp}"`
- **时间戳新鲜度检查：** 签名必须在 5 分钟窗口内，防止重放攻击
- **防止未授权密钥更新：** 发现 Agent DID 的攻击者无法在不持有原始私钥的情况下注册新密钥

这确保了 Agent 身份——及其附带的 Trust Score 历史——不会被第三方窃取或劫持。

### 8.5 API 密钥管理

Agent 到服务器的身份验证使用结构化的 API 密钥系统：

- **密钥格式：** `ak_live_{random}` — 人类可识别的前缀标识密钥类型
- **存储：** SHA-256 哈希存储（原始密钥永远不在服务器上存储）
- **密钥轮换：** 支持带审计轨迹的密钥轮换（`rotated_from` 字段将新密钥链接到前任）
- **分级限速：** 免费版 = 10 次/分钟，专业版 = 60 次/分钟，企业版 = 300 次/分钟
- **密钥前缀追踪：** 允许在不暴露完整密钥的情况下进行识别和撤销
...
| 5 | 2026 Q1 | ✅ 完成 | 框架适配器、PyPI/npm 发布、Prometheus |
| 6 | 2026 Q1 | ✅ 完成 | Go SDK、MCP 服务器、查询与审计引擎、BIP39 身份恢复、证明包、OpenClaw 扫描器、OpenTelemetry 自动插桩、Agent 注册安全、API 密钥管理、1,059 项测试 |
| 7 | 2026 Q1-Q2 | 🔄 进行中 | 白皮书 v3.0、IETF/W3C 准备、反滥用框架、正式安全审计 |
| 8 | 2026 Q2-Q3 | 计划中 | 公开发布、Base 主网锚定、首批外部集成、LLaChat v1.0 |
| 9 | 2026 Q3-Q4 | 计划中 | AIP（Agent 身份协议）+ ASP（Agent 安全协议） |
| 10 | 2027 | 计划中 | ACP（Agent 认证协议）+ 欧盟 AI 法案合规工具包 |
| 11 | 2027-2028 | 愿景 | PAP（遗体 Agent 协议）、Agent 保险基础设施、A2A 信任框架 |

### 13.2 当前实现状态

| 组件 | 版本 | 测试数 | 状态 |
|------|------|--------|------|
| Python SDK | v0.10.0 | 835 | 已发布到 PyPI |
| TypeScript SDK | v0.3.0 | 43 | 已发布到 npm |
| Go SDK | v0.1.0 | 50 | CLI 工具：atlast-go |
| ECP 服务器 | v1.0.0 | 131 | 已部署在 api.weba0.com |
| **合计** | | **1,059** | CI 全部通过 |
...
协议已上线。SDK 已发布。1,059 项测试在 Python、TypeScript、Go 和服务器中全部通过。标准是开放的。
...
|------|------|------|
| `GET` | `/health` | 健康检查 |
| `GET` | `/.well-known/ecp.json` | ECP 发现文档 |
| `GET` | `/v1/discovery/agents` | 列出已注册 Agent |
| `GET` | `/v1/discovery/agents/{did}/stats` | Agent 统计信息 |
| `GET` | `/v1/stats` | 平台统计信息 |
| `POST` | `/v1/batches` | 上传 Merkle 批次 |
| `GET` | `/v1/batches/{batch_id}` | 获取批次状态 |
| `POST` | `/v1/agents/register` | 注册 Agent + 获取 API 密钥 |
| `POST` | `/v1/auth/rotate-key` | 轮换 API 密钥 |
| `GET` | `/v1/auth/me` | 获取当前 Agent 信息 |
| `GET` | `/v1/verify/{attestation_uid}` | 验证证明 |
| `POST` | `/v1/verify/merkle` | 验证 Merkle 证明 |
| `GET` | `/v1/attestations` | 列出区块链证明 |
| `GET` | `/v1/attestations/{batch_id}` | 获取批次的证明 |
| `GET` | `/v1/agents/{did}/drift` | Agent 行为漂移分析 |
| `GET` | `/v1/agents/{did}/records` | Agent ECP 记录 |
| `GET` | `/v1/super-batches/{id}` | 获取超级批次状态 |
...
| `speed_anomaly` | 输出长度/延迟比超过阈值（如 <100ms 内 600+ 字符） | 负面 | 响应生成速度不合理地快——可能表示缓存/预制输出而非真正的计算 |
...
| **BIP39** | 比特币改进提案 39——用于确定性 Agent 身份恢复的助记短语标准 |
...
| **MCP** | 模型上下文协议——将工具暴露给 AI Agent 的标准；ATLAST MCP 服务器提供 8 个 ECP 工具 |
| **Merkle 证明** | O(log N) 的证明，证明特定元素存在于 Merkle 树中 |
| **OpenClaw 扫描器** | 扫描 OpenClaw Agent 会话日志并自动创建 ECP 记录的工具 |
| **PAP** | 遗体 Agent 协议——主人去世后 Agent 资产/身份继承的框架 |
| **证明包** | 自包含的、可独立验证的证明捆绑包，支持选择性披露 |
